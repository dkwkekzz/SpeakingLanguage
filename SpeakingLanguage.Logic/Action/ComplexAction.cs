...
namespace SpeakingLanguage.Logic
{
    ...
    {
        public static IAction CreateNewAction(ConditionAttribute condAttr, MethodInfo mth)
        {
            int argsCount = condAttr.SrcType.Length;
            ...
            if (targetAttr != null)
            {
                argsCount += targetAttr.SrcType.Length;
            }

            var typeArgs = new Type[argsCount];
            var condLength = condAttr.SrcType.Length;
            for (int i = 0; i != condLength; i++)
                typeArgs[i] = condAttr.SrcType[i];
            
            if (targetAttr != null)
            {
                var targetLength = targetAttr.SrcType.Length;
                for (int i = 0; i != targetLength; i++)
                    typeArgs[condLength + i] = targetAttr.SrcType[i];
            }
            ...
            action.Initialize(mth, condAttr.SrcType, targetAttr.SrcType);
